...
class AbstractSendMessage(ABC):
    def _save_notifications(self) -> list[Notifications]:
        ...
        notifications_list = [
            Notifications(user=user, content=self.message, type=self.type) for user in self.users
        ]
        notifications = Notifications.objects.bulk_create(notifications_list)
